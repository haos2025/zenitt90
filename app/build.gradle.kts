...
dependencies {
    ...
    // DI — Hilt (миграция с ServiceLocator, см. NEXT_STEPS.md / REFACTOR_PROMPT.md)
    ...
    // NanoHTTPD вместо ktor-server-cio — промт прямо просил "не тащить
    // тяжёлый фреймворк ради пары эндпоинтов", NanoHTTPD — один небольшой
    // jar без движка/Netty-подобных транзитивных зависимостей. Телефон
    // шлёт запрос уже подключённым OkHttp — отдельный HTTP-клиент на
    // телефон не нужен.
