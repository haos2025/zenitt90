...
data class PlaylistSourceEntity(
    ...
    // "vod" | "live". Имеет смысл только для type == "m3u" — плоский
    // M3U-список не различает VOD и живой эфир структурно (в отличие от
    // Xtream, где это разные API-эндпоинты), поэтому пользователь явно
    // указывает при добавлении источника, что это плейлист живых каналов.
    // Игнорируется для type == "xtream": там refresh() всегда фетчит и
    // VOD (get_vod_streams), и live (get_live_streams) независимо от
